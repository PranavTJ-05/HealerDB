...
async def get_stream_stats():
    ...
    try:
        ...
        streams = {
            "healerdb:events": settings.redis_stream_name,
            "healerdb:repair": settings.redis_repair_stream,
            "healerdb:apply": settings.redis_apply_stream,
            "healerdb:escalation": settings.redis_escalation_stream,
        }
...
async def get_escalations(limit: int = Query(default=20, le=100)):
    ...
    These are events that HealerDB couldn't auto-heal — need human review.
    """
    try:
        r = aioredis.Redis(
            host=settings.redis_host,
