    ...
    try:
        async with _registry_engine.begin() as conn:
            await conn.execute(text("""
                CREATE TABLE IF NOT EXISTS _healerdb_connections (
            ...
            await conn.execute(text("""
                ...
                ON _healerdb_connections (connection_id)
            """))
        logger.info("[Registry] Table _healerdb_connections ready")
...
async def save_connection(conn_obj: DatabaseConnection) -> bool:
    ...
    try:
        async with _registry_engine.begin() as conn:
            await conn.execute(
                text("""
                    INSERT INTO _healerdb_connections (
    ...
    try:
        async with _registry_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    SELECT * FROM _healerdb_connections
