...
async def init_reports_store():
    """Create _healerdb_reports table + indexes if they don't exist. Called on startup."""
    ...
    try:
        async with _reports_engine.begin() as conn:
            await conn.execute(text("""
                CREATE TABLE IF NOT EXISTS _healerdb_reports (
            ...
            await conn.execute(text("""
                ...
                ON _healerdb_reports (event_id)
            """))

            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_reports_table_fqn
                ON _healerdb_reports (table_fqn)
            """))

            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_reports_created_at
                ON _healerdb_reports (created_at DESC)
            """))

            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_reports_column_anomaly
                ON _healerdb_reports (table_name, column_name, anomaly_type)
            """))

        logger.info("[ReportsStore] Table _healerdb_reports ready")
...
async def get_recurrence_count(
    ...
    try:
        async with _reports_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    SELECT COUNT(*) FROM _healerdb_reports
                    WHERE table_name  = :table_name
