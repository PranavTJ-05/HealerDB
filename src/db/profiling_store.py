    ...
    try:
        async with _profiling_engine.begin() as conn:
            await conn.execute(text("""
                CREATE TABLE IF NOT EXISTS _healerdb_profiling_reports (
            ...
            await conn.execute(text("""
                ...
                ON _healerdb_profiling_reports (report_id)
            """))
            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_profiling_created_at
                ON _healerdb_profiling_reports (created_at DESC)
            """))
        logger.info("[ProfilingStore] Table _healerdb_profiling_reports ready")
...
async def save_report(report: ProfilingReport) -> bool:
    ...
    try:
        ...
        async with _profiling_engine.begin() as conn:
            await conn.execute(
                text("""
                    INSERT INTO _healerdb_profiling_reports (
    ...
    try:
        async with _profiling_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    SELECT report_data FROM _healerdb_profiling_reports
        ...
        async with _profiling_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    ...
                    FROM _healerdb_profiling_reports
