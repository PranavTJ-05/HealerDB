# HealerDB — Auto-Documentation Feature
## Technical Reference Document

> **Author:** Session documentation — HealerDB Hackathon  
> **Date:** April 25, 2026  
> **Status:** Production-ready, fully tested  
> **Scope:** End-to-end design, implementation, testing, and known limitations of the Auto-Documentation feature built on top of HealerDB v0.4.0
...
4. [Phase 0 — FixReport Storage (`_healerdb_reports`)](#4-phase-0--fixreport-storage)
...
Every time HealerDB fixes a data quality issue, that knowledge dies silently. It lives in a proposal that gets approved and forgotten. Nobody updates the data catalog. Nobody writes down what was wrong, what the fix was, or whether this has happened before.
...
**The Auto-Documentation feature closes this loop.** When HealerDB successfully applies a fix to production, three things happen automatically with no human involvement:

1. A structured `FixReport` record is written to `_healerdb_reports` in Postgres
2. The fixed column's description in OpenMetadata is annotated with the fix note, and the table is tagged `HealerDB.healed`
3. The Slack resolved card is updated with links to the Incident Timeline, the Audit Entry, and the OpenMetadata table page

Every table HealerDB has ever touched becomes a self-documenting asset. Every fix becomes a timestamped incident report. No tickets, no backlog, no tribal knowledge lost.
...
apply.py (_apply_live — post-COMMIT only)
    ...
    └── if action == APPLIED:
            ...
            ├── build FixReport(...)
            │
            ├── [1] write_report()          → _healerdb_reports (Postgres)
            │
...
**`recurrence_count` is the critical field.** It is computed at write time by querying `_healerdb_reports` for prior rows matching the same `(table_name, column_name, anomaly_type)`. A value of `0` means first occurrence. A value of `3` means this is the 4th time this exact issue has been fixed. This number feeds the frontend recurrence pill and the Slack card context.
...
| Function | Purpose |
|---|---|
| `init_reports_store()` | Creates `_healerdb_reports` table + 4 indexes. Called at startup. |
...
The reports store connects to `TARGET_DB_NAME` from `.env`. In the Northwind setup this is `northwind`, not `healerdb`. All `_healerdb_*` tables live in the `northwind` database. When connecting via psql for debugging, always use `-d northwind`.

```bash
docker exec -it healerdb_postgres psql -U healerdb_user -d northwind -c "SELECT * FROM _healerdb_reports ORDER BY created_at DESC LIMIT 5;"
```
...
**`ensure_healerdb_tag()`** — called once at boot in `main.py`.
- Creates `HealerDB` classification in OM if not exists
- Creates `HealerDB.healed` tag under that classification if not exists
- Idempotent — safe to call on every restart
- Tag creation payload uses `"classification": "HealerDB"` (string, not object) — OM 1.6 API requirement
...
- Check if `HealerDB.healed` already applied — skip if so (idempotent)
- PATCH tags array with new tag appended to existing tags
- Tag payload: `{"tagFQN": "HealerDB.healed", "source": "Classification", "labelType": "Automated", "state": "Confirmed"}`
...
```
**[HealerDB 2026-04-25 14:32 UTC]** Fixed `null_violation` (UPDATE) — 
...
try:
    tag_ready = await om_client.ensure_healerdb_tag()
    if tag_ready:
        logger.info("[Boot] OM HealerDB.healed tag ready")
    else:
...
URL construction:
```python
base_frontend = slack_settings.healerdb_base_url.replace("8001", "3000")
base_om       = slack_settings.healerdb_base_url.replace("8001", "8585")
...
| `src/services/om_client.py` | Modified | Added 4 new methods: `ensure_healerdb_tag`, `patch_column_description`, `add_table_tag`, `annotate_fix` |
...
3. StreamConsumer reads event
   ...
   → RepairProposalRecord created in _healerdb_proposals
   → Published to healerdb:slack
        ↓
4. Human approves via frontend or Slack
   → POST /proposals/{id}/approve
   → RepairDecision published to healerdb:repair
        ↓
5. RepairAgent reads healerdb:repair
   → Re-runs sandbox validation
   → Stores fix in ChromaDB (healerdb_fixes)
   → Publishes RepairDecision to healerdb:apply
        ↓
6. ApplyAgent reads healerdb:apply
...
7. [AUTO-DOC BLOCK — new]
   → extract_primary_column(fix_sql)
   → get_recurrence_count(table, col, type)   ← queries _healerdb_reports
   → build FixReport(...)
   → write_report()                            → _healerdb_reports row
...
```sql
CREATE TABLE _healerdb_reports (
    id                  SERIAL PRIMARY KEY,
    report_id           VARCHAR(36)   NOT NULL UNIQUE,  -- UUID
    event_id            VARCHAR(36)   NOT NULL,          -- FK to _healerdb_audit
...
CREATE INDEX idx_reports_event_id    ON _healerdb_reports (event_id);
CREATE INDEX idx_reports_table_fqn   ON _healerdb_reports (table_fqn);
CREATE INDEX idx_reports_created_at  ON _healerdb_reports (created_at DESC);
CREATE INDEX idx_reports_column_anomaly ON _healerdb_reports (table_name, column_name, anomaly_type);
```
...
### Pre-flight
- [ ] Docker running: `healerdb_postgres`, `healerdb_redis`, `openmetadata_server`, `openmetadata_elasticsearch`
- [ ] Boot log shows `[Boot] Reports store ready`
- [ ] Boot log shows `[Boot] OM HealerDB.healed tag ready`
...
- [ ] `SELECT * FROM _healerdb_reports` shows 1 row in `northwind` database
...
- [ ] Log shows `[OM] Added HealerDB.healed tag`
- [ ] `localhost:8585` → orders table → ship_region column has HealerDB fix note
- [ ] orders table has `HealerDB.healed` tag badge
...
> "HealerDB has resolved 3 incidents, healed 516 rows, at 95% average confidence. Look at this — 67% recurrence rate. Two thirds of the fixes are issues we've already seen before."

Point to the second card with the violet pill:
> "This column has failed the same test 3 times. HealerDB knows this. It documents it automatically — no engineer wrote any of this."
...
Show the `HealerDB.healed` tag on the table — searchable across the entire catalog.

**Step 4 — Show the intelligence layer (30 seconds)**

Go back to `localhost:3000/incidents`. Point to the new card's recurrence pill.
> "That's the 4th occurrence. This number feeds back into the diagnosis engine. HealerDB will eventually skip human approval for patterns it's solved before, with this history as the evidence."

**Closing line:**
> "No tickets, no backlog, no tribal knowledge lost. Every table HealerDB touches becomes a self-documenting asset."