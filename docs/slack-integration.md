# HealerDB Slack Bot Integration — Technical Documentation
...
The HealerDB Slack bot is a **conversational database reliability engineer** embedded in Slack. It is not a notification bot — it is a full interactive agent that:
...
│                                                                 │
│  rejection_store.py ──── ChromaDB ──── healerdb_rejections      │
└─────────────────────────────────────────────────────────────────┘
```
...
- The same Redis instance (different stream: `healerdb:slack`)
...
├── rejection_store.py       # ChromaDB wrapper for healerdb_rejections collection
...
Called by `src/services/stream_consumer._save_proposal()` after a proposal is created. Publishes to `healerdb:slack` stream.
...
Collection: `healerdb_rejections`  
Model: `all-MiniLM-L6-v2` (same as `healerdb_fixes`)  
...
│     ├── Redis ping
│     ├── XGROUP CREATE healerdb:slack slack-bot (idempotent)
...
│     └── get_or_create_collection(healerdb_rejections)
│
└── asyncio.gather(
...
[Boot] HealerDB Slack Bot ready ✓
[SlackListener] Listening on 'healerdb:slack'
Bolt app is running!
```
...
All calls go to `{healerdb_base_url}/api/v1/`:
...
|---|---|---|---|
| `healerdb:events` | webhook.py | stream_consumer.py | Main pipeline events |
| `healerdb:repair` | stream_consumer | repair_agent | Repair tasks |
| `healerdb:apply` | repair_agent | apply_agent | Apply tasks |
| `healerdb:escalation` | stream_consumer | (manual) | Low confidence events |
| `healerdb:slack` | slack_notifier | stream_listener | **Bot notifications** |

The bot only reads/writes `healerdb:slack`. It never touches the main pipeline streams.
...
|---|---|---|
| `healerdb_fixes` | Backend `vector_store.py` | Successful fix history for RAG |
| `healerdb_rejections` | Bot `rejection_store.py` | Engineer rejection reasons |
...
Key: `healerdb:slack:map:{proposal_id}` with 24h TTL. Allows thread Q&A to survive bot restarts.
