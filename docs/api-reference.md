# HealerDB API Reference

This document covers the public HTTP endpoints currently exposed by the HealerDB FastAPI service.
...
Onboard a database into HealerDB. The API validates the credentials immediately, then runs metadata registration, ingestion, and profiling in the background.
