...
> HealerDB detects data quality failures, diagnoses root cause using AI, proposes fixes for human review, and applies them to production — safely, with a full audit trail.

**Frontend:** [PranavTJ-05/healerdb-frontend](https://github.com/PranavTJ-05/healerdb-frontend) &nbsp;|&nbsp; **Demo:** [Watch on YouTube](https://www.youtube.com/watch?v=H3tvkptnSZ4)

---

### Dashboard Overview
![HealerDB Dashboard](./assets/hero_section.png)
...
HealerDB closes the loop automatically:

1. OpenMetadata fires a webhook when a test fails
2. HealerDB classifies the failure, diagnoses the root cause with **Groq LLM** and a RAG knowledge base
...
5. On approval, HealerDB applies the fix to production inside a transaction with post-apply assertions
...
No tickets. No tribal knowledge lost. Every table HealerDB has ever touched becomes a self-documenting asset.
...
### System Architecture
![HealerDB Architecture](./assets/architecture.svg)
...
**Full audit trail**
Every pipeline outcome — applied, dry run, rolled back, skipped, escalated — is written to `_healerdb_audit` with the exact SQL that ran, row counts, assertion results, and confidence score.
...
```bash
git clone https://github.com/your-org/healerdb.git
cd healerdb
```
...
| `REDIS_CONSUMER_GROUP` | `healerdb-agents` | | |
| `APP_HOST` | `0.0.0.0` | | |
...
```bash
psql postgresql://healerdb_user:healerdb_pass@localhost:5433/northwind \
...
OpenMetadata runs a data quality test and finds a NULL violation in `employees.region`. It fires a webhook to HealerDB at `POST /webhook/om-test-failure`. HealerDB returns 200 immediately and processes the event in the background.
...
If confidence ≥ 0.70, HealerDB spins up an ephemeral Postgres container, seeds it with up to 500 rows from the target table, runs the fix, validates assertions, and captures a before/after data diff. A proposal is created in `pending_approval` status.
...
│   ├── db/
│   │   ├── audit_log.py         # _healerdb_audit CRUD
│   │   ├── event_store.py       # _healerdb_events CRUD
│   │   ├── vector_store.py      # ChromaDB wrapper
│   │   ├── profiling_store.py   # _healerdb_profiling_reports CRUD
│   │   ├── proposal_store.py    # _healerdb_proposals CRUD
...
HealerDB isn't just a notification bot — it's a conversational SRE embedded in your workspace.
