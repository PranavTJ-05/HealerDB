...
class RepairAgent:
    """
    Reads the healerdb:repair stream.
    For each repairable diagnosis:
      1. Run sandbox (with retries)
      2. If passes → publish to healerdb:apply
      3. If all retries fail → publish to healerdb:escalation
    """
...
def _fallback_event_from_diagnosis(
    ...
    """
    Fallback when _healerdb_events lookup misses.
