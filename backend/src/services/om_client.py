...
class OpenMetadataClient:
    ...
    async def ensure_healerdb_tag(self) -> bool:
        """
        One-time bootstrap: ensure the HealerDB classification and 
        ...
        try:
            ...
            resp = await self._client.get(
                "/api/v1/classifications/name/HealerDB",
                headers=headers,
            )
            ...
            if resp.status_code == 404:
                ...
                resp = await self._client.post(
                    ...
                    content='{"name":"HealerDB","description":"Tags applied automatically by the HealerDB self-healing system.","mutuallyExclusive":false}',
                )
                if resp.status_code not in (200, 201):
                    logger.warning(
                        f"[OM] Could not create HealerDB classification: "
                        f"{resp.status_code} {resp.text[:200]}"
                    )
                    return False
                logger.info("[OM] Created HealerDB classification")
            else:
                logger.info("[OM] HealerDB classification already exists")

            # 2. Check if 'healed' tag exists under HealerDB
            resp = await self._client.get(
                "/api/v1/tags/name/HealerDB.healed",
                headers=headers,
            )
            ...
            if resp.status_code == 404:
                ...
                resp = await self._client.post(
                    "/api/v1/tags",
                    headers={**headers, "Content-Type": "application/json"},
                    content='{"name":"healed","description":"This table was automatically healed by HealerDB.","classification":"HealerDB"}'
                )
                if resp.status_code not in (200, 201):
                    logger.warning(
                        f"[OM] Could not create HealerDB.healed tag: "
                        f"{resp.status_code} {resp.text[:200]}"
                    )
                    return False
                logger.info("[OM] Created HealerDB.healed tag")
            else:
                logger.info("[OM] HealerDB.healed tag already exists")

            return True

        except Exception as e:
            logger.warning(f"[OM] ensure_healerdb_tag failed (non-fatal): {e}")
            return False
    ...
    async def add_table_tag(self, table_fqn: str) -> bool:
        """
        Add the HealerDB.healed tag to a table entity.
        ...
        try:
            ...
            already_tagged = any(
                t.get("tagFQN") == "HealerDB.healed"
                for t in existing_tags
            )
            if already_tagged:
                logger.info(
                    f"[OM] Table {table_fqn} already has HealerDB.healed tag"
                )
                return True
            ...
            if resp.status_code in (200, 201):
                logger.info(f"[OM] Added HealerDB.healed tag to {table_fqn}")
                return True
            else:
                logger.warning(
