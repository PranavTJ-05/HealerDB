...
async def init_event_store():
    """
    Create _healerdb_events table on startup.
    ...
    try:
        async with _event_engine.begin() as conn:
            await conn.execute(text("""
                CREATE TABLE IF NOT EXISTS _healerdb_events (
            ...
            await conn.execute(text("""
                ...
                ON _healerdb_events (event_id)
            """))
            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_events_created_at
                ON _healerdb_events (created_at DESC)
            """))
        logger.info("[EventStore] Table _healerdb_events ready")
    ...
    try:
        async with _event_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    SELECT event_data FROM _healerdb_events
