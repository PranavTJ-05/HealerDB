    ...
    try:
        async with _proposal_engine.begin() as conn:
            await conn.execute(text("""
                CREATE TABLE IF NOT EXISTS _healerdb_proposals (
            ...
            await conn.execute(text("""
                ...
                ON _healerdb_proposals (proposal_id)
            """))
            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_proposals_status
                ON _healerdb_proposals (status)
            """))
            await conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_proposals_created_at
                ON _healerdb_proposals (created_at DESC)
            """))
        logger.info("[ProposalStore] Table _healerdb_proposals ready")
...
async def create_proposal(proposal: RepairProposalRecord) -> bool:
    ...
    try:
        async with _proposal_engine.begin() as conn:
            await conn.execute(
                text("""
                    INSERT INTO _healerdb_proposals (
    ...
    try:
        async with _proposal_engine.connect() as conn:
            result = await conn.execute(
                text("""
                    SELECT * FROM _healerdb_proposals
                    WHERE proposal_id = :proposal_id
                """),
                {"proposal_id": proposal_id},
            )
