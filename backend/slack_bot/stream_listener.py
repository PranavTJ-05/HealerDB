...
class SlackStreamListener:
    ...
    async def _process_message(self, msg_id: str, fields: dict):
        """
        Handle a single healerdb:slack stream message.
    ...
    async def _handle_new_proposal(self, fields: dict):
        ...
        2. Fetch full proposal from HealerDB API
        ...
        post_resp = await self._slack.chat_postMessage(
            channel=channel,
            text=f"🔴 HealerDB detected an anomaly on `{table_name}` — building proposal...",
        ...
        if owner_uid and owner_uid != "U00000000":
            try:
                await self._slack.chat_postMessage(
                    channel=owner_uid,
                    text=(
                        f"👋 Hey — HealerDB generated a fix proposal for "
    ...
    def _build_doc_links(
        ...
        """
        base_frontend = slack_settings.healerdb_base_url.replace(
            "8001", "3000"
        )
        base_om = slack_settings.healerdb_base_url.replace(
            "8001", "8585"
        )
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    ...
                    proposal_resp = await client.get(
                        f"{slack_settings.healerdb_base_url}/api/v1/proposals/{proposal_id}"
                    )
                    ...
                    if event_id:
                        audit_resp = await client.get(
                            f"{slack_settings.healerdb_base_url}/api/v1/audit/{event_id}"
                        )
                    ...
                    if action == "applied":
                        reports_resp = await client.get(
                            f"{slack_settings.healerdb_base_url}/api/v1/reports",
    ...
    async def fetch_table_history(self, table_name: str) -> str:
        ...
        try:
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if not reports:
                    return f"📋 No fix history found for `{table_name}`. It has not been healed by HealerDB yet."
    ...
    async def _fetch_proposal(self, proposal_id: str) -> dict | None:
        """
        Fetch full proposal detail from HealerDB API.
        GET /api/v1/proposals/{proposal_id}
        """
        url = f"{slack_settings.healerdb_base_url}/api/v1/proposals/{proposal_id}"
        proposal = None
        try:
