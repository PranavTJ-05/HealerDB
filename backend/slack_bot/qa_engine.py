...
def build_system_prompt() -> str:
    ...
    return """You are HealerDB Assistant, an expert database reliability engineer embedded in Slack.
...
async def answer_question(question: str, proposal_id: str) -> str:
    ...
    if not proposal:
        return "❌ Could not retrieve proposal details. Check the HealerDB dashboard."
...
async def answer_why_table(table_name: str) -> str:
    ...
    if not rejections:
        return (
            ...
            f"or it's the first time HealerDB has seen it."
        )
