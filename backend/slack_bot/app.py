"""
HealerDB Slack Bot — complete app.
...
BASE_URL = f"{slack_settings.healerdb_base_url}/api/v1"
...
async def handle_approve(ack, body, client, action):
    ...
    if not result:
        await client.chat_postEphemeral(
            channel=channel_id, user=user_id,
            text="❌ Approval API call failed. Check HealerDB backend logs.",
        )
        return
...
async def handle_rejection_submit(ack, body, client, view):
    ...
    if not result:
        await client.chat_postEphemeral(
            ...
            text="❌ Rejection API call failed. Check HealerDB backend logs.",
        )
        return
    ...
    await client.chat_postMessage(
        ...
        blocks=[
            ...
            },
            {
                ...
                "elements": [{
                    "type": "mrkdwn",
                    "text": f"HealerDB · grounded in proposal `{proposal_id[:8]}...`, fix history, and rejection memory",
                }],
            },
        ],
    )
...
async def _cmd_status(say):
    ...
    if not status_data:
        await say("❌ Could not reach HealerDB backend.")
        return
    ...
    if streams_data and "streams" in streams_data:
        ...
        for stream_key, stream_val in streams_data["streams"].items():
            ...
            short_name = stream_key.replace("healerdb:", "")
    ...
    blocks.append({
        ...
        "elements": [{
            "type": "mrkdwn",
            "text": f"HealerDB · {_utc_now()}",
        }],
    })

    await say(
        blocks=blocks,
        text=f"HealerDB: {healthy}/{total} stages healthy · {mode}",
    )
...
async def _cmd_ask(say, question: str):
    ...
    await say(
        blocks=[
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "🤖  HealerDB Answer", "emoji": True},
            },
            {
                "type": "section",
        ...
        blocks=[
            ...
            },
            {
                ...
                "elements": [{
                    "type": "mrkdwn",
                    "text": (
                        f"Source: `healerdb_rejections` ChromaDB collection · "
...
async def main():
    logger.info("=" * 55)
    logger.info("HealerDB Slack Bot starting")
    logger.info(f"  Channel  : #{slack_settings.slack_ops_channel}")
    logger.info(f"  Backend  : {slack_settings.healerdb_base_url}")
    ...
    logger.info("[Boot] HealerDB Slack Bot ready ✓")
